require 'rails_helper'

RSpec.describe "TODO Items API" do
    # # Create Test data 
    # let!(:todo) { create(:todo) }
    # let!(:items) { create_list(:todo, 20, todo_id: todo.id) }
    # let(:todo_id) { todo.id }
    # let(:id) { items.first.id }

    # # Test suite for GET /todos/:todo_id/items
    # describe "GET /todos/todo_id/items" do
    #     before { get "/todos/#{todo_id}/items" }

    #     context "When todo exists" do
    #         it 'return status code 200' do
    #             expect(response).to have_http_status(200)
    #         end
    #         it 'returns all todo items' do
    #             expect(json.size).to eq(20)
    #         end
    #     end

    #     context "When todo does not exists" do
    #         let(:todo_item) {0}

    #         it 'return status code 404' do
    #             expect(response).to have_http_status(404)
    #         end

    #         it 'returns a not found message' do
    #             expect(response.body).to match(/Couldn't find Todo/)
    #         end
    #     end
    # end

    # # Test suite to GET /todos/:todo_id/items/:id
    # describe "GET /todos/:todo_id/items/:id" do
    #     before { get "/todos/#{todo_id}/items/#{id}" }

    #     context "When Todo Item exists" do
    #         it 'returns status code 200' do
    #             expect(response).to have_http_status(200)
    #         end
    #         it 'returns the item' do
    #             expect(json['id']).to eq(id)
    #         end
    #     end

    #     context "When todo item does not exists" do
    #         let(:id) { 0 }

    #         it 'returns status code 404' do
    #             expect(response).to have_http_status(404)
    #         end
            
    #         it 'returns a not found message' do
    #             expect(response.body).to match(/Couldn't find todo item/)
    #         end
    #     end
    # end

    # # Test suite for POST /todos/:todo_id/items
    # describe "PUT /todos/:todo_id/items" do
    #     let(:valid_attributes) { { name: 'Visit Narnia', done: false } }
    #     context "When request attributes are valid" do
    #         before { post "/todos/#{todo_item}/items", params: valid_attributes }

    #         it 'returns status code 201' do
    #             expect(response).to have_http_status(201)
    #         end
    #     end
    #     context "When request attributes are invalid" do
    #         before { post "/todos/#{todo_item}/items", params: {} }
    #         it 'return status code 422' do
    #             expect(response).to have_http_status(422)
    #         end
    #         it 'returns a failure message' do
    #             expect(response).to match(/Validation failed: Name can't be blank/)
    #         end
    #     end
    # end
    
    # # Test suite for PUT /todos/:todo_id/items/:id
    # describe "PUT /todos/:todo_item/items/:id" do
    #     let(:valid_attributes) { { name: 'Haider' } }
    #     before { put "/todos/#{todo_id}/items/#{id}" }
    #     context "When item exists" do
    #         it 'returns status code 204' do
    #             expect(response).to have_http_status(204)
    #         end
    #         it 'updates the item' do
    #             updated_item = Item.find(id)
    #             expect(updated_item.name).to eq('Haider')
    #         end
    #     end

    #     context "When item does not exists" do
    #         let(:id) { 0 }
    #         it 'returns status code 404' do
    #             expect(response).to have_http_status(404)
    #         end
    #         it 'returns failure message' do
    #             expect(response).to match(/Couldn't find Item/)
    #         end    
    #     end
    # end

    # # Test suite for DELETE /todos/todo_id/items/:id
    # describe "DELETE /todos/todo_id/items/:id" do
    #     before { delete "/todos/#{todo_id}/items/#{id}"}
    #     it 'returns status code 204' do
    #         expect(response).to have_http_status(204)
    #     end  
    # end    
end